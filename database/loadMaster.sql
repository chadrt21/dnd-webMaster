...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`NoteFolder` (
  `noteFolderID` INT UNSIGNED NOT NULL,
...
USE `dungeonBuddiesDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dungeonBuddiesDB`.`NoteFolder_BEFORE_UPDATE` BEFORE UPDATE ON `NoteFolder` FOR EACH ROW
BEGIN
	DECLARE fp TEXT;
