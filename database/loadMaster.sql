...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`DM` (
  ...
  `spotifyAccessToken` TEXT NULL DEFAULT NULL,
  `spotifyRefreshToken` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`dmID`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`Campaign` (
  ...
  `campaignLogoURL` TEXT NULL,
  `layoutData` JSON NULL,
  `settingData` JSON NULL,
...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`Location` (
  ...
  INDEX `location_campaign_fk_idx` (`campaignID` ASC),
  CONSTRAINT `location_campaign_fk`
...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`Subklass` (
  ...
  `subklassFlavor` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`WeaponProperty` (
  ...
  `weaponPropertyName` VARCHAR(45) NOT NULL,
...
-- -----------------------------------------------------
-- Table `dungeonBuddiesDB`.`NoteFolder`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`NoteFolder` (
  `noteFolderID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `campaignID` INT UNSIGNED NOT NULL,
  `parentID` INT UNSIGNED NULL,
  `folderName` VARCHAR(45) NULL,
  `filepath` TEXT NULL,
  `isDeleted` TINYINT(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`noteFolderID`),
  INDEX `notefolder_notefolder_fk_idx` (`parentID` ASC),
  INDEX `notefolder_campaign_fk_idx` (`campaignID` ASC),
  CONSTRAINT `notefolder_notefolder_fk`
    FOREIGN KEY (`parentID`)
    REFERENCES `dungeonBuddiesDB`.`NoteFolder` (`noteFolderID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `notefolder_campaign_fk`
    FOREIGN KEY (`campaignID`)
    REFERENCES `dungeonBuddiesDB`.`Campaign` (`campaignID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `dungeonBuddiesDB`.`Note`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`Note` (
  ...
  `isDeleted` TINYINT(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`noteID`),
  INDEX `note_campaign_fk_idx` (`campaignID` ASC),
  INDEX `note_notefolder_fk_idx` (`folderID` ASC),
  CONSTRAINT `note_campaign_fk`
    FOREIGN KEY (`campaignID`)
    REFERENCES `dungeonBuddiesDB`.`Campaign` (`campaignID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `note_notefolder_fk`
...
-- -----------------------------------------------------
-- Table `dungeonBuddiesDB`.`CampaignHasSpotifyPlaylist`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`CampaignHasSpotifyPlaylist` (
  `campaignID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `spotifyUri` VARCHAR(255) NOT NULL,
  `type` VARCHAR(45) NULL,
  `hotkey` VARCHAR(45) NULL,
  `playlistName` TEXT NULL,
  PRIMARY KEY (`campaignID`, `spotifyUri`),
  CONSTRAINT `campaignhasspotifyplaylist_campaign_fk`
    FOREIGN KEY (`campaignID`)
    REFERENCES `dungeonBuddiesDB`.`Campaign` (`campaignID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `dungeonBuddiesDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dungeonBuddiesDB`.`NoteFolder_BEFORE_INSERT`
	BEFORE INSERT ON `NoteFolder`
	FOR EACH ROW
BEGIN
	DECLARE fp TEXT;
	IF NEW.parentID <> NULL THEN
		SET fp = (SELECT filepath 
			FROM NoteFolder
            WHERE noteFolderID = NEW.parentID);
        
		SET NEW.filepath = CONCAT(fp, NEW.folderName, '/ ');
	ELSE SET NEW.filepath = CONCAT(NEW.folderName, '/ ');
	END IF;
END$$

USE `dungeonBuddiesDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `dungeonBuddiesDB`.`NoteFolder_BEFORE_UPDATE` 
	BEFORE UPDATE ON `NoteFolder` 
	FOR EACH ROW
BEGIN
	DECLARE fp TEXT;
	IF NEW.folderName <> OLD.folderName THEN
		SET fp = (SELECT filepath 
			FROM NoteFolder
            WHERE noteFolderID = NEW.parentID);
