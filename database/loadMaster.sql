...
-- -----------------------------------------------------
-- Table `dungeonBuddiesDB`.`NoteList`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `dungeonBuddiesDB`.`NoteList` (
  `campaignID` INT UNSIGNED NOT NULL,
  `noteID` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`noteID`, `campaignID`),
  INDEX `campaignlist_campaign_fk_idx` (`campaignID` ASC),
  CONSTRAINT `notelist_campaign_fk`
    FOREIGN KEY (`campaignID`)
    REFERENCES `dungeonBuddiesDB`.`Campaign` (`campaignID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `notelist_note_fk`
    FOREIGN KEY (`noteID`)
    REFERENCES `dungeonBuddiesDB`.`Note` (`noteID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
